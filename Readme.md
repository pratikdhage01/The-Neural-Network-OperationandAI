...
flowchart LR
  ...
  UI -->|HTTP| API[FastAPI :8000]

  API -->|/api/chat/*| Graph[LangGraph Agent Orchestration]
  ...
  Twilio[Twilio WhatsApp] -->|webhook| API
  ngrok[ngrok tunnel] -. exposes .-> API
```
...
```mermaid
flowchart TD
  Start([User message]) --> Router[Router: route_message]

  Router -->|customer_agent| Customer[Customer Agent<br/>product inquiries & FAQs]
  Router -->|order_agent| Order[Order Agent<br/>placement & tracking]
  Router -->|inventory_agent| Inventory[Inventory Agent<br/>stock checks]
  Router -->|supplier_agent| Supplier[Supplier Agent<br/>procurement]
  Router -->|workload_agent| Workload[Workload Agent<br/>task assignment]
  Router -->|bottleneck_agent| Bottleneck[Bottleneck Agent<br/>detect issues]
